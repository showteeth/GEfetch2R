...
ShowCBDatasets <- function(lazy = TRUE, json.folder = NULL, update = FALSE, quiet = FALSE, timeout = 3600) {
  ...
  dts.df <- ParseCBdataset(datasets.files)
...
#'
#' @param meta Metadata used to load, can be obtained with \code{ExtractCBDatasets}.
#' Skip when \code{link} is not NULL. Default: NULL.
#' @param link Vector contains dataset/collection link(s), e.g. "https://cells.ucsc.edu/?ds=adult-ureter".
...
#' Default: NULL (access json files online).
#' @return Dataframe contains sample information and cell type composition.
#' @importFrom jsonlite fromJSON flatten
#' @importFrom data.table rbindlist
#' @importFrom dplyr select distinct relocate
#' @importFrom magrittr %>%
#' @export
#'
#' @examples
...
#' }
ExtractCBComposition <- function(meta = NULL, link = NULL, json.folder = NULL) {
  # prepare samples for download
  if (is.null(meta)) {
    if (is.null(link)) {
      stop("The meta and link are NULL, please provide at least one valid value!")
    } else {
      meta <- ParseCBlink(link)
    }
  }
  # prepare json files
  if (!is.null(json.folder)) {
    message("Load the local json files!")
    # check json files
    expect.json.files <- file.path(json.folder, meta$name, "dataset.json")
    ...
    if (length(valid.json.files) == 0) {
      stop("No valid json files under: ", json.folder, ". Please check!")
    }
    names(valid.json.files) <- gsub(pattern = paste0(json.folder, "/", "(.*?)", "/dataset.json"), replacement = "\\1", x = valid.json.files)
  } else {
    valid.json.files <- file.path("https://cells.ucsc.edu/", meta$name, "dataset.json")
    names(valid.json.files) <- meta$name
  }
  # get cell type composition
  cell.type.list <- lapply(names(valid.json.files), function(x) {
    ct.json <- jsonlite::fromJSON(txt = valid.json.files[x])
  ...
  cell.type.df <- merge(meta, cell.type.df, by = "name", all.y = TRUE) %>%
    as.data.frame() %>%
    dplyr::select(-c("matrix", "barcode", "feature", "matrixType", "unit", "coords", "name"))
  if ("subLabel" %in% colnames(cell.type.df)) {
...
#'
#' @param meta Metadata used to load, can be obtained with \code{ExtractCBDatasets},
#' should contain name, matrixType, matrix, barcode, feature, coords columns.
#' Skip when \code{link} is not NULL. Default: NULL.
#' @param link Vector contains dataset/collection link(s), e.g. "https://cells.ucsc.edu/?ds=adult-ureter".
#' Skip when \code{meta} is not NULL. Default: NULL.
...
#' }
ParseCBDatasets <- function(meta = NULL, link = NULL, timeout = 1000, obs.value.filter = NULL,
                            obs.keys = NULL, include.genes = NULL, merge = TRUE) {
  # prepare samples for download
  if (is.null(meta)) {
    if (is.null(link)) {
      stop("The meta and link are NULL, please provide at least one valid value!")
  ...
  # check columns
  CheckColumns(df = meta, columns = c("name", "matrixType", "matrix", "barcode", "feature", "coords"))
  # set timeout
  env.timeout <- getOption("timeout")
  on.exit(options(timeout = env.timeout)) # restore timeout
  options(timeout = timeout)
  ...
  base.url <- "https://cells.ucsc.edu/"
  seu.obj.list <- list()
  for (rn in 1:nrow(meta)) {
    sample.name <- gsub(pattern = "/", replacement = "_", x = meta[rn, "name"])
    matrix.type <- meta[rn, "matrixType"]
    # prepare exp matrix
    exp.url <- paste0(base.url, meta[rn, "name"], "/", meta[rn, "matrix"])
    # prepare barcode and feature
    if (matrix.type == "matrix") {
      barcode.url <- NULL
      feature.url <- NULL
    } else if (matrix.type == "10x") {
      barcode.url <- paste0(base.url, meta[rn, "name"], "/", meta[rn, "barcode"])
      feature.url <- paste0(base.url, meta[rn, "name"], "/", meta[rn, "feature"])
    }
    # prepare metadata
    meta.url <- paste0(base.url, meta[rn, "name"], "/meta.tsv")
    # prepare coord
    coord.file <- paste0(base.url, meta[rn, "name"], "/", strsplit(x = meta[rn, "coords"], split = ", ")[[1]])
