...
ExtractObject <- function(obj, obj.class, slot) {
  if (obj.class == "Seurat") {
    ...
    seu.install <- as.character(utils::packageVersion("Seurat"))
    # check object Seurat version
    seu.version <- as.character(SeuratObject::Version(obj))
    # get all assays
    all.assays <- SeuratObject::Assays(obj)
    if (grepl(pattern = "^5", seu.install)) {
      message("Detect Seurat version: ", seu.version, ", with assay(s): ", paste(all.assays, collapse = ", "), ".")
      ...
      if (grepl(pattern = "^5", seu.version)) {
        message(
          "The object is generated by Seurat v5 (", seu.version, ") but the installed version is a previous version: ", seu.install,
          ". The count matrix may not be extracted correctly (depends on the assay version used)."
        )
      } else {
        message("Detect Seurat version: ", seu.version, ", with assay(s): ", paste(all.assays, collapse = ", "), ".")
      }
